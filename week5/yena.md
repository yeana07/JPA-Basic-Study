...
ORM 에서 **객체의 참조**와 **테이블의 외래키**를 **매핑**하기 중요.
...
- 테이블 연관관계
  ...
  - 회원 <-> 팀 테이블은 양방향 관계. (외래키 하나로 양 방향 조인 가능)
- 객체 연관관계와 테이블 연관관계 차이점
  - 참조를 통한 연관관계는 언제나 단방향이다. 양방향으로 만들려면 양쪽에 필드를 추가해 참조를 보관해야 한다.  
    **-> 양방향이 아닌 단방향 관계 2개**
...
2. 객체지향 쿼리 사용 JPQL
   ...
   List<Member> resultList = em.createQuery(jpql, Member,class)
    ...
    for (Member meemeber : resultList) {
...
mappedBy속성으로 양방향 매핑일 때 반대쪽 매핑의 필드 이름을 값으로 사용한다. (Member.team)
...
이런 차이로 JPA는 두 객체 연관관계 중 하나를 정해 테이블의 외래키를 관리한다.
...
회원 -> 팀 연관관계에서 회원이 주인일 때 Team.members에만 값을 저장하고 Member.team에는 아무 값도 입력하지 않으면 외래키 null 값이 저장됨.
...
비즈니르 로직에 따라 반대 방향 객체그래프탐색이 필요할 때만 양방향을 사용하도록 코드를 추가해도 된다.
